...
def savepic(itemid,eventid):
  ...
  data = {"itemids":'{0}'.format(itemid),"period":"3600","stime":"0","width":"800"}
  data = parse.urlencode(data).encode('utf-8')
  req = urllib.request.Request(zabbix_server_charturl, data=data)
  opener = urllib.request.build_opener(urllib.request.HTTPCookieProcessor(cookie))
  response = opener.open(req)
  req = urllib.request.Request(url='{1}/{0}.png'.format(eventid,ttserver), data=response.read(),method='PUT')
...
postdata={"host_group":"{0}".format(host_group),"eventid":"{0}".format(eventid),"alarm_title":"{0}".format(alarm_title),"triggerdescription":"{0}".format(triggerdescription),"host_name":"{0}".format(host_name),"triggerseverity":"{0}".format(triggerseverity),"alarm_status":"{0}".format(alarm_status),"hostconn":"{0}".format(hostconn)}
